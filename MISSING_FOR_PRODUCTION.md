...
- **Vercel lead validation повністю виправлено (2026-06-04)**: ✅ Vercel webpack кеш тримав старий скомпілований bundle для `/api/leads` (11-source enum) ігноруючи всі зміни коду. Справжня причина: `API_URL` вказував на `lider-avtoschool-**dev**` Firebase замість production. Рішення: задеплоєно Firebase Functions на dev проект + `next.config.ts` rewrite. Заявки зі всіх CTA тепер проходять: `sticky_mobile`, `hero_cta`, `floating_phone`, `branch_card`, `footer`, `about`, `unknown_*` → `website`.
