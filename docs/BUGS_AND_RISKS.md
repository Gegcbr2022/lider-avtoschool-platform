...
бейдж-фикс (`0e13fe9`), Crashlytics (`e34e3fe`), hero главной (`eb6891f`), game loop + агрегация
рейтинга (`9f36600`). `typecheck` (root + apps/mobile) — exit 0. Что сделано/осталось:
...
- 👤 Блокеры релиза без изменений: App Check debug-token (B-02), prod keystore (B-03), эквайринг (B-04),
  ротация утёкших ключей (см. ниже). Это действия владельца, не код.
